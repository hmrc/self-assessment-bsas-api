...
class SubmitSelfEmploymentBsasController @Inject()(val authService: EnrolmentsAuthService,
  ...
  private def errorResult(errorWrapper: ErrorWrapper) = {
    errorWrapper.error match {
      case BadRequestError | NinoFormatError | BsasIdFormatError | RuleIncorrectOrEmptyBodyError | MtdErrorWithCustomMessage(
            FormatAdjustmentValueError.code) | MtdErrorWithCustomMessage(RuleAdjustmentRangeInvalid.code) =>
      ...
      case RuleSummaryStatusInvalid | RuleSummaryStatusSuperseded | RuleBsasAlreadyAdjusted | RuleOverConsolidatedExpensesThreshold |
          RuleTradingIncomeAllowanceClaimed | RuleNotSelfEmployment | RuleErrorPropertyAdjusted =>
