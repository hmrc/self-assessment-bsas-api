...
import v1.models.request.submitBsas.ukProperty.{SubmitUKPropertyBsasRequestBody, SubmitUkPropertyBsasRequestData}
...
class SubmitUkPropertyBsasService @Inject()(connector: SubmitUkPropertyBsasConnector) extends DesResponseMappingSupport with Logging {
  ...
  def submitPropertyBsas(request: SubmitUkPropertyBsasRequestData)(
      ...
      correlationId: String): Future[Either[ErrorWrapper, ResponseWrapper[SubmitUkPropertyBsasResponse]]] = {
    ...
    val result = for {
      ...
      mtdResponseWrapper <- EitherT.fromEither[Future](
        validateSubmitUkPropertyBsasSuccessResponse(desResponseWrapper, Some(retrieveTypeOfBusiness(request.body))))
