...
object Versions {
  ...
  def getFromRequest(implicit hc: HeaderCarrier): Option[String] = getFrom(hc.headers)

  def getFromRequest(request: RequestHeader): Option[String] = getFrom(request.headers.headers)
