...
object NumberValidation {
  ...
  def validate(field: BigDecimal, path: String, min: BigDecimal = 0, max: BigDecimal = 99999999999.99): List[MtdError] =
    doValidate(field, path, min, max, disallowZero = false)

  def validateAdjustment(field: Option[BigDecimal],
                         path: String,
  ...
  private def doValidate(field: BigDecimal, path: String, min: BigDecimal, max: BigDecimal, disallowZero: Boolean): List[MtdError] = {
