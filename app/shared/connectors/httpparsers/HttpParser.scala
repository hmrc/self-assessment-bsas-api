...
trait HttpParser extends Logging {
  ...
  private val multipleHipErrorReads: Reads[Seq[HipDownstreamErrorCode]] = (__ \ "failures").read[Seq[HipDownstreamErrorCode]]
  ...
  def parseErrors(response: HttpResponse): DownstreamError = {
    ...
    val singleHipError      = response.validateJson[HipDownstreamErrorCode].map(err => DownstreamErrors(List(err.toDownstreamErrorCode)))
