...
@Singleton
class SubmitSelfEmploymentBsasNrsProxyConnector @Inject()(http: HttpClient, appConfig: AppConfig) {
  ...
  def submit[T](nino: String, requestBody: SubmitSelfEmploymentBsasRequestBody)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Unit] = {
    ...
    http
      .POST[SubmitSelfEmploymentBsasRequestBody, Unit](s"${appConfig.mtdNrsProxyBaseUrl}/mtd-api-nrs-proxy/$nino/itsa-annual-adjustment", requestBody)
  }
}
