...
class RetrieveSelfEmploymentBsasValidatorFactorySpec extends UnitSpec {
  ...
  private def validatorFor(taxYear: String) =
    new RetrieveSelfEmploymentBsasValidatorFactory().validator(nino = "ignoredNino", calculationId = "ignored", taxYear = taxYear)
  ...
  "RetrieveForeignPropertyBsasValidatorFactory" when {
    "given a request corresponding to a Def1 schema" should {
      "return a Def1 validator" in {
        validatorFor("2023-24") shouldBe a[Def1_RetrieveSelfEmploymentBsasValidator]
      }
    }
