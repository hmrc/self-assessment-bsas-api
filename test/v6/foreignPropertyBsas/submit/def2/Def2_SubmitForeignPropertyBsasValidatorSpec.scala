...
class Def2_SubmitForeignPropertyBsasValidatorSpec extends UnitSpec with JsonErrorValidators {
  ...
  private def foreignPropertyBodyWith(nonFhlEntries: JsValue*): JsObject =
    Json
      .parse(
        s"""{
  ...
  private val parsedForeignPropertyBody    = foreignPropertyBody.as[Def2_SubmitForeignPropertyBsasRequestBody]
    ...
    "return the parsed domain object" when {
      ...
      "passed a valid foreign property request" in {
        ...
        result shouldBe Right(
          Def2_SubmitForeignPropertyBsasRequestData(parsedNino, parsedCalculationId, None, parsedForeignPropertyBody)
        )
      }
      ...
      "passed a valid non-fhl consolidated expenses request" in {
        ...
        val parsedForeignPropertyBodyConsolidated = foreignPropertyBodyConsolidated.as[Def2_SubmitForeignPropertyBsasRequestBody]

        val result = validator(validNino, validCalculationId, None, foreignPropertyBodyConsolidated).validateAndWrapResult()

        result shouldBe Right(
          Def2_SubmitForeignPropertyBsasRequestData(parsedNino, parsedCalculationId, None, parsedForeignPropertyBodyConsolidated)
        )
      }
      ...
      "a minimal foreign property request is supplied" in {
        val minimalForeignPropertyBody = Json.parse("""
          ...
          |}
          |""".stripMargin)
        val parsedMinimalForeignPropertyBody = minimalForeignPropertyBody.as[Def2_SubmitForeignPropertyBsasRequestBody]

        val result = validator(validNino, validCalculationId, None, minimalForeignPropertyBody).validateAndWrapResult()

        result shouldBe Right(
          Def2_SubmitForeignPropertyBsasRequestData(parsedNino, parsedCalculationId, None, parsedMinimalForeignPropertyBody)
        )
      }
      ...
      "a valid request with a taxYear is supplied" in {
        ...
        val parsedForeignPropertyBodyWithConsolidatedEntry = foreignPropertyBodyWithConsolidatedEntry.as[Def2_SubmitForeignPropertyBsasRequestBody]
        ...
        result shouldBe Right(
          Def2_SubmitForeignPropertyBsasRequestData(
            parsedNino,
            parsedCalculationId,
            Some(parsedTaxYear),
