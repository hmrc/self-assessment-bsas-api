...
class Def2_SubmitForeignPropertyBsasRequestBodySpec extends UnitSpec {
  ...
  private val parsedEmptyRequestBody = Def2_SubmitForeignPropertyBsasRequestBody(None)

  private val parsedForeignPropertyRequestBody =
    Def2_SubmitForeignPropertyBsasRequestBody(Some(List(ForeignProperty("FRA", None, None))))
      ...
      "return the expected foreign Property data object" in {
        Json
          .parse("""
            ...
            |}
            |""".stripMargin)
          .as[Def2_SubmitForeignPropertyBsasRequestBody] shouldBe parsedForeignPropertyRequestBody
      }
    }

    "given a full foreign property body" should {
      "return the expected foreign Property data object" in {
        mtdRequestForeignPropertyFull.as[Def2_SubmitForeignPropertyBsasRequestBody] shouldBe requestForeignPropertyFull
      }
    }
