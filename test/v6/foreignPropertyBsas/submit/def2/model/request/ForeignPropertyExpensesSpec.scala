...
import v5.foreignPropertyBsas.submit.def2.model.request.ForeignPropertyExpenses

class ForeignPropertyExpensesSpec extends UnitSpec {

  val expenses: ForeignPropertyExpenses = ForeignPropertyExpenses(
      ...
      "return the corresponding model" in {
        Json
          .parse("""
            ...
            |}
            |""".stripMargin)
          .as[ForeignPropertyExpenses] shouldBe expenses
      }
    }
  ...
  "writes" when {
    "passed a model" should {
      "return the downstream JSON" in {
        Json.toJson(expenses) shouldBe Json.parse("""
            |{
            |  "premisesRunningCosts": 1.12,
            |  "repairsAndMaintenance": 2.12,
            |  "financialCosts": 3.12,
