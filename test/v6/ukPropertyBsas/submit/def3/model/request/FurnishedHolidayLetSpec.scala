...
class FurnishedHolidayLetSpec extends UnitSpec {
  ...
  private val furnishedHolidayLet: request.FurnishedHolidayLet = FurnishedHolidayLet(
    Some(FHLIncome(None)),
    Some(FHLExpenses(None, None, None, None, None, None, None, None))
  )

  private val emptyFurnishedHolidayLet: request.FurnishedHolidayLet = FurnishedHolidayLet(None, None)

  "reads" when {
    "given MTD json" should {
      "return the expected FurnishedHolidayLet" in {
        json.as[request.FurnishedHolidayLet] shouldBe furnishedHolidayLet
      }
    }

    "given an empty JSON object" should {
      "return an empty FurnishedHolidayLet" in {
        JsObject.empty.as[request.FurnishedHolidayLet] shouldBe emptyFurnishedHolidayLet
      }
    }
  }
