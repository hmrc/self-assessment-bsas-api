...
class SubmitForeignPropertyBsasValidatorFactorySpec extends UnitSpec with JsonErrorValidators {
  ...
  private val parsedNonFhlBody    = nonFhlBody.as[SubmitForeignPropertyBsasRequestBody]
    ...
    "return the parsed domain object" when {
      ...
      "passed a valid fhl request" in {
        ...
        result shouldBe Right(
          SubmitForeignPropertyBsasRequestData(parsedNino, parsedCalculationId, None, parsedFhlBody)
        )
      }
      ...
      "passed a valid non-fhl request" in {
        ...
        result shouldBe Right(
          SubmitForeignPropertyBsasRequestData(parsedNino, parsedCalculationId, None, parsedNonFhlBody)
        )
      }
      ...
      "passed a valid fhl consolidated expenses request" in {
        ...
        result shouldBe Right(
          SubmitForeignPropertyBsasRequestData(parsedNino, parsedCalculationId, None, parsedFhlBodyConsolidated)
        )
      }

      "passed a valid non-fhl consolidated expenses request" in {
        val nonFhlBodyConsolidated       = nonFhlBodyWith(entryConsolidated)
        val parsedNonFhlBodyConsolidated = nonFhlBodyConsolidated.as[SubmitForeignPropertyBsasRequestBody]

        val result = validator(validNino, validCalculationId, None, nonFhlBodyConsolidated).validateAndWrapResult()

        result shouldBe Right(
          SubmitForeignPropertyBsasRequestData(parsedNino, parsedCalculationId, None, parsedNonFhlBodyConsolidated)
        )
      }
      ...
      "a minimal fhl request is supplied" in {
        ...
        val parsedMinimalFhlBody = minimalFhlBody.as[SubmitForeignPropertyBsasRequestBody]

        val result = validator(validNino, validCalculationId, None, minimalFhlBody).validateAndWrapResult()

        result shouldBe Right(
          SubmitForeignPropertyBsasRequestData(parsedNino, parsedCalculationId, None, parsedMinimalFhlBody)
        )
      }
      ...
      "a minimal non-fhl request is supplied" in {
        val minimalNonFhlBody       = Json.parse("""
          ...
          |}
          |""".stripMargin)
        val parsedMinimalNonFhlBody = minimalNonFhlBody.as[SubmitForeignPropertyBsasRequestBody]

        val result = validator(validNino, validCalculationId, None, minimalNonFhlBody).validateAndWrapResult()

        result shouldBe Right(
          SubmitForeignPropertyBsasRequestData(parsedNino, parsedCalculationId, None, parsedMinimalNonFhlBody)
        )
      }

      "a valid request with a taxYear is supplied" in {
        val nonFhlBodyWithConsolidatedEntry       = nonFhlBodyWith(entryConsolidated)
        val parsedNonFhlBodyWithConsolidatedEntry = nonFhlBodyWithConsolidatedEntry.as[SubmitForeignPropertyBsasRequestBody]
        ...
        result shouldBe Right(
          SubmitForeignPropertyBsasRequestData(parsedNino, parsedCalculationId, Some(parsedTaxYear), parsedNonFhlBodyWithConsolidatedEntry)
        )
      }
    }
