      ...
      "both expenses are present" when {
        ...
        "return an error" when {
          "a model with consolidatedExpenses and costOfGoodsAllowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model.copy(costOfGoodsAllowable = Some(123.45)), None, path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and paymentsToSubcontractorsAllowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model.copy(paymentsToSubcontractorsAllowable = Some(123.45)), None, path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and wagesAndStaffCostsAllowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model.copy(wagesAndStaffCostsAllowable = Some(123.45)), None, path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and carVanTravelExpensesAllowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model.copy(carVanTravelExpensesAllowable = Some(123.45)), None, path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and premisesRunningCostsAllowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model.copy(premisesRunningCostsAllowable = Some(123.45)), None, path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and maintenanceCostsAllowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model.copy(maintenanceCostsAllowable = Some(123.45)), None, path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and adminCostsAllowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model.copy(adminCostsAllowable = Some(123.45)), None, path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and interestOnBankOtherLoansAllowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model.copy(interestOnBankOtherLoansAllowable = Some(123.45)), None, path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and financeChargesAllowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model.copy(financeChargesAllowable = Some(123.45)), None, path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and irrecoverableDebtsAllowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model.copy(irrecoverableDebtsAllowable = Some(123.45)), None, path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and professionalFeesAllowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model.copy(professionalFeesAllowable = Some(123.45)), None, path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and depreciationAllowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model.copy(depreciationAllowable = Some(123.45)), None, path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and otherExpensesAllowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model.copy(otherExpensesAllowable = Some(123.45)), None, path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and advertisingCostsAllowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model.copy(advertisingCostsAllowable = Some(123.45)), None, path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and businessEntertainmentCostsAllowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model.copy(businessEntertainmentCostsAllowable = Some(123.45)), None, path) shouldBe List(error)
          }
        }
        ...
        "return an error" when {
          "a model with consolidatedExpenses and costOfGoodsDisallowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model, Some(additionModel.copy(costOfGoodsDisallowable = Some(123.45))), path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and paymentsToSubcontractorsDisallowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model, Some(additionModel.copy(paymentsToSubcontractorsDisallowable = Some(123.45))), path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and wagesAndStaffCostsDisallowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model, Some(additionModel.copy(wagesAndStaffCostsDisallowable = Some(123.45))), path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and carVanTravelExpensesDisallowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model, Some(additionModel.copy(carVanTravelExpensesDisallowable = Some(123.45))), path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and premisesRunningCostsDisallowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model, Some(additionModel.copy(premisesRunningCostsDisallowable = Some(123.45))), path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and maintenanceCostsDisallowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model, Some(additionModel.copy(maintenanceCostsDisallowable = Some(123.45))), path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and adminCostsDisallowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model, Some(additionModel.copy(adminCostsDisallowable = Some(123.45))), path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and interestOnBankOtherLoansDisallowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model, Some(additionModel.copy(interestOnBankOtherLoansDisallowable = Some(123.45))), path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and financeChargesDisallowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model, Some(additionModel.copy(financeChargesDisallowable = Some(123.45))), path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and irrecoverableDebtsDisallowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model, Some(additionModel.copy(irrecoverableDebtsDisallowable = Some(123.45))), path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and professionalFeesDisallowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model, Some(additionModel.copy(professionalFeesDisallowable = Some(123.45))), path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and depreciationDisallowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model, Some(additionModel.copy(depreciationDisallowable = Some(123.45))), path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and otherExpensesDisallowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model, Some(additionModel.copy(otherExpensesDisallowable = Some(123.45))), path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and advertisingCostsDisallowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model, Some(additionModel.copy(advertisingCostsDisallowable = Some(123.45))), path) shouldBe List(error)
          }
          "a model with consolidatedExpenses and businessEntertainmentCostsDisallowable is supplied" in {
            BothExpensesValidation.bothExpensesValidation(
              model, Some(additionModel.copy(businessEntertainmentCostsDisallowable = Some(123.45))), path) shouldBe List(error)
          }
        }
      }
    }
