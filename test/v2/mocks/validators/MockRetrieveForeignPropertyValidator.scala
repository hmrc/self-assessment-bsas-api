...
class MockRetrieveForeignPropertyValidator extends MockFactory {
  ...
  object MockValidator {
    def validate(data: RetrieveForeignPropertyRawData): CallHandler1[RetrieveForeignPropertyRawData, List[MtdError]] = {
      (mockValidator.validate(_: RetrieveForeignPropertyRawData))
        .expects(data)
    }
  }
}