  ...
  "validate" when {
    ...
    "validating adjustments" when {
      "min and max are not specified" must {
        ...
        "allow -99999999999.99" in {
          NumberValidation.validateAdjustment(Some(-99999999999.99), path) shouldBe Nil
        }

        "disallow less than -99999999999.99" in {
          NumberValidation.validateAdjustment(Some(-100000000000.00), path) shouldBe List(error)
        }
        ...
        "not allow 0" in {
          NumberValidation.validateAdjustment(Some(0), path) shouldBe List(error)
        }

        "allow None" in {
          NumberValidation.validateAdjustment(None, path) shouldBe Nil
        }
      }
      ...
      "min and max are specified" must {
        ...
        val error = ValueFormatError.copy(paths = Some(Seq(path)), message = "The value must be between -10 and 10")
        "allow no more than max" in {
          NumberValidation.validateAdjustment(Some(10.01), path, min, max) shouldBe List(error)
        }

        "allow no less than min" in {
          NumberValidation.validateAdjustment(Some(-10.01), path, min, max) shouldBe List(error)
        }
      }
    }
