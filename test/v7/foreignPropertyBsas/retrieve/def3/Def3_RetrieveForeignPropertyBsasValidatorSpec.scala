...
class Def3_RetrieveForeignPropertyBsasValidatorSpec extends UnitSpec {
  ...
  private def validator(nino: String, calculationId: String, taxYear: String) =
    new Def3_RetrieveForeignPropertyBsasValidator(nino, calculationId, taxYear)
  ...
  "validator" should {
    "return the parsed domain object" when {
      "passed a valid request" in {
        ...
        result shouldBe Right(
          Def3_RetrieveForeignPropertyBsasRequestData(parsedNino, parsedCalculationId, parsedTaxYear)
        )
      }
    ...
    "return a single error" when {
      ...
      "passed an invalid calculation id" in {
        val result = validator(validNino, "not-a-calculation-id", validTaxYear).validateAndWrapResult()
        result shouldBe Left(
