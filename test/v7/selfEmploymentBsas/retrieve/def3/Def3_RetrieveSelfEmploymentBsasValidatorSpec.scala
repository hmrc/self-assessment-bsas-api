...
class Def3_RetrieveSelfEmploymentBsasValidatorSpec extends UnitSpec {
  ...
  private def validator(nino: String, calculationId: String, taxYear: String) =
    new Def3_RetrieveSelfEmploymentBsasValidator(nino, calculationId, taxYear)
    ...
    "return the parsed domain object" when {
      "passed a valid non-TYS request" in {
        ...
        result shouldBe Right(
          Def3_RetrieveSelfEmploymentBsasRequestData(parsedNino, parsedCalculationId, parsedTaxYear)
        )
      }
